...
//SELECT u.Mail, u.Nombre,x.Stock, x.Precio FROM Vendedor as v, Usuario as u,Vehiculos as x WHERE v.Mail = "ripazha.darom@gmail.com" and u.Mail = "ripazha.darom@gmail.com"
            conexion.query ('SELECT u.Mail, u.Nombre,x.Stock, x.Precio FROM Vendedor as v, Usuario as u,Vehiculos as x WHERE v.Mail = "'+decodificada2.id+'" and u.Mail = "'+decodificada2.id+'"', (error, results)=>{
                //console.log(results)
...
exports.isAuthenticatedG = async (req, res, next)=>{
    if(req.cookies.jwt){
        try {
            ...
            conexion.query ('SELECT u.Mail, u.Nombre,x.Stock,x.Precio FROM Gerente as g, Usuario as u,Vehiculos as x WHERE g.Mail = "'+decodificada3.id+'" and u.Mail = "'+decodificada3.id+'"', (error, results)=>{
                if(!results){return next()}
                req.user = results
                return next()
            })
